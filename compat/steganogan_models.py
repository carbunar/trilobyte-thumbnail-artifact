...
# Compatibility patch for legacy SteganoGAN checkpoints that serialize
# old optimizer objects.
if not hasattr(Optimizer, "_trilobyte_compat_patch"):
    _orig_optimizer_setstate = Optimizer.__setstate__
    ...
    def _patched_optimizer_setstate(self, state):
        ...
        if not hasattr(self, "param_groups"):
            self.param_groups = []
        return _orig_optimizer_setstate(self, state)

    Optimizer.__setstate__ = _patched_optimizer_setstate
    Optimizer._trilobyte_compat_patch = True
...
class SteganoGAN(object):
    ...
    def decode(self, image):
        ...
        candidates = Counter()
        bits = [1 if bit else 0 for bit in image.data.cpu().numpy().tolist()]
